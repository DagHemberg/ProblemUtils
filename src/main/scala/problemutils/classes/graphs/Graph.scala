...
trait Graph[V]:
  ...
  def pathsFrom(start: V) = Pathfinder.dijkstra(start)
  def pathBetween(start: V, end: V, heuristic: V => Double = _ => 0) = Pathfinder.aStar(start, end, heuristic)
